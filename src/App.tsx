...
function Code({
  ...
  return (
    <div className="rounded-xl border border-[#2A2722] bg-[#0A0A0B] font-mono text-sm leading-relaxed text-[#D4C5B0] shadow-inner">
      ...
      <pre
        className={`overflow-x-auto p-4 ${wrap ? "whitespace-pre-wrap break-words" : "whitespace-pre"}`}
      >
        {children}
      </pre>
    </div>
...
const TOOL_GROUPS = [
  {
    ...
    tools: [
      [
        ...
        "Start here — full project context (tasks, decisions, facts, skills, team)",
      ],
      [
        'context(action="summary", project_id)',
        "Project status and milestone snapshot",
      ],
      [
        'context(action="changes", project_id, since)',
        "Grouped audit changes since an ISO timestamp",
      ],
      [
      ...
      [
        ...
        "Intent-aware top-k tool-action shortlist",
      ],
    ],
  },
  {
  ...
  },
  {
    ...
    tools: [
      [
        ...
        "List tasks with advanced filters and optional JSON output",
      ],
      [
        'tasks(action="create", project_id, title, ...)',
        "Create a task with optional priority/estimate/milestone/assignee",
      ],
      [
        'tasks(action="update", task_id, ...)',
        "Update task lifecycle: status, priority, description, or metadata",
      ],
      [
        'tasks(action="context", task_id)',
        "Task details, comments, dependencies, and linked decisions",
      ],
      [
        'tasks(action="batch_create", project_id, items=[{title, ...}, ...])',
        "Create multiple tasks in one request (requires items[].title)",
      ],
      [
        'tasks(action="batch_update", updates=[{id, ...}, ...])',
        "Bulk update multiple tasks in one request (requires updates[].id)",
      ],
      [
  ...
  },
  {
    ...
    tools: [
      [
        'tasks(action="list_milestones", project_id)',
        "List delivery milestones for a project",
      ],
      [
        'tasks(action="get_milestone", milestone_id)',
        "Get milestone details and progress",
      ],
      [
        'tasks(action="create_milestone", project_id, title, status?)',
        "Create a new milestone",
      ],
      [
        'tasks(action="update_milestone", milestone_id, title?, status?)',
        "Update milestone details",
      ],
      ['tasks(action="delete_milestone", milestone_id)', "Delete a milestone"],
      [
        'tasks(action="reorder_milestones", project_id, milestone_ids=[])',
        "Change the sort order of milestones",
      ],
    ],
  },
  {
    ...
    tools: [
      [
        ...
        "Manage decisions, facts, and skills (entity = decision|fact|skill)",
      ],
    ],
  },
  {
    ...
    tools: [
      [
        ...
        "List versioned documents (draft, spec, report, review, code)",
      ],
      [
        'files(action="get", file_id, version?)',
        "Get latest or specific version of a file",
      ],
      [
        'files(action="create", project_id, title, file_type, body, ...)',
        "Create a new versioned file linked to a project or entity",
      ],
      [
  ...
  },
  {
    ...
    tools: [
      [
        ...
        "Team members, agent discovery, and messaging",
      ],
    ],
  },
];
...
const TOOL_PARAM_DETAILS_OVERRIDES: Record<string, ToolParam[]> = {
  ...
  'context(action="changes", project_id, since)': [
    ...
    {
      name: "since",
      description: "ISO-8601 timestamp (e.g., 2026-03-21T00:00:00Z).",
    },
  ],
  'context(action="search", project_id, q, limit?)': [
  ...
  'context(action="shortlist", q, limit?, full_tool_mode?)': [
    ...
    { name: "q", description: "Natural language intent query." },
    {
      name: "limit",
      optional: true,
      description: "Number of ranked operations to return.",
    },
    {
      name: "full_tool_mode",
      optional: true,
      description: "Return full tool definitions if true.",
    },
  ],
  'tasks(action="list", project_id, status?, q?, q_any?, q_all?, q_not?, response_mode?)':
    [
      ...
      },
      {
        name: "q_any",
        optional: true,
        description: "Matches if ANY provided term matches.",
      },
      {
        name: "q_all",
        optional: true,
        description: "Matches if ALL provided terms match.",
      },
      {
        name: "q_not",
        optional: true,
        description: "Excludes tasks matching any provided term.",
      },
      {
        name: "response_mode",
        optional: true,
        description: "human | json | both.",
      },
    ],
  'tasks(action="create", project_id, title, ...)': [
    ...
    },
    {
      name: "estimate",
      optional: true,
      description: "Integer estimate (e.g., story points).",
    },
    {
      name: "milestone_id",
      optional: true,
      description: "UUID of the linked milestone.",
    },
    {
      name: "assignee_id",
      optional: true,
      description: "UUID of the assigned agent or user.",
    },
  ],
  "knowledge(entity, action, ...)": [
    ...
    { name: "action", description: "list, get, create, update, or delete." },
    {
      name: "project_id",
      optional: true,
      description: "Required for decision/fact list/create.",
    },
    {
      name: "item_id",
      optional: true,
    ...
    { name: "title", optional: true, description: "Title field." },
    {
      name: "body",
      optional: true,
      description: "Markdown body content (facts/skills).",
    },
    {
      name: "rationale",
      optional: true,
    ...
    },
    {
      name: "task_id",
      optional: true,
      description: "Linked task for decisions.",
    },
  ...
  "collaboration(action, ...)": [
    {
      name: "action",
      description:
        "list_team_members, discover_agents, get_agent_activity, send_message, get_messages, update_my_card, join_team.",
    },
    {
      name: "recipient_id",
      optional: true,
      description: "Target UUID for send_message.",
    },
    { name: "body", optional: true, description: "Message or profile body." },
    {
      name: "since",
      optional: true,
      description: "Activity filter timestamp.",
    },
  ],
};
...
export default function App() {
  ...
  const [expandedToolParams, setExpandedToolParams] = useState<
    ...
    <div className="min-h-screen font-sans selection:bg-[#B09E80] selection:text-[#0A0A0B]">
      ...
      <header className="sticky top-0 z-20 border-b border-[#2A2722] bg-[#0A0A0B]/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <a
            href={MARKETING_URL}
            className="flex items-center gap-3 font-display text-xl font-medium text-[#F0E6D2]"
          >
            <img
              src="/favicon.svg"
              alt="ProjectBrain"
              className="h-7 w-7 filter sepia-[0.5] saturate-50"
            />
            Docs
          </a>
          <div className="flex items-center gap-6">
            <a
              href={MARKETING_URL}
              className="hidden text-sm text-[#D4C5B0] opacity-60 hover:opacity-100 transition-opacity sm:block"
            >
              Home
            </a>
            <a
            ...
            <a
              href={APP_URL}
              className="hidden rounded-lg bg-[#B09E80] px-4 py-2 text-sm font-semibold text-[#0A0A0B] hover:bg-[#D4C5B0] transition-colors sm:block"
            >
              Open App
            </a>
            {/* Mobile menu toggle */}
      ...
      <div className="mx-auto flex max-w-7xl">
        ...
        <aside className="sticky top-[69px] hidden h-[calc(100vh-69px)] w-64 shrink-0 overflow-y-auto border-r border-[#2A2722] py-10 pr-4 pl-6 sm:block">
          <nav className="space-y-6">
            {SECTION_GROUPS.map((group) => (
              <div key={group.label}>
                <p className="mb-1 px-4 text-xs font-bold uppercase tracking-[0.15em] text-[#B09E80] opacity-40">
                  {group.label}
                </p>
          ...
          <div className="fixed inset-x-0 top-[69px] z-10 border-b border-[#2A2722] bg-[#0A0A0B] px-6 py-6 sm:hidden">
            <nav className="space-y-4">
                  ...
                  {group.items.map((s) => (
                    <button
                      ...
                      className={`block w-full rounded-lg px-4 py-3 text-left text-base ${
                        active === s.id
                          ? "bg-[#B09E80]/10 font-semibold text-[#F0E6D2]"
                          : "text-[#D4C5B0] opacity-60"
                      }`}
                    >
                      {s.label}
                    </button>
              ...
              <div className="mt-6 flex flex-col gap-4 border-t border-[#2A2722] pt-6">
                <a
                  href={MARKETING_URL}
                  className="text-base text-[#D4C5B0] opacity-60"
                >
                  Home
                </a>
                <a
                ...
                <a
                  href={APP_URL}
                  className="rounded-lg bg-[#B09E80] py-3 text-center text-base font-semibold text-[#0A0A0B]"
                >
                  Open App
                </a>
              </div>
        ...
        <main className="min-w-0 flex-1 px-6 py-12 sm:px-12 lg:px-16 animate-fade-up">
          ...
          <section id="getting-started" className="mb-24">
            <h1 className="font-display text-4xl font-medium text-[#F0E6D2] md:text-5xl tracking-tight">
              Getting Started
            </h1>
            <p className="mt-4 text-lg text-[#D4C5B0] opacity-70 leading-relaxed font-sans max-w-2xl">
              Connect your AI agent to ProjectBrain in under a minute.
              You&apos;ll need a free account and an API key.
            </p>

            <div className="mt-12 space-y-12">
              <div className="group border-l border-[#2A2722] pl-8 py-2 hover:border-[#B09E80] transition-colors">
                <h3 className="font-display text-2xl font-medium text-[#F0E6D2]">
                  1. Create an account
                </h3>
                <p className="mt-3 text-[#D4C5B0] opacity-70 leading-relaxed font-sans">
                  ...
                  <a
                    href={APP_URL}
                    className="text-[#B09E80] hover:underline transition-colors"
                  >
                    {APP_URL}
                  </a>{" "}
                  using GitHub or Google. A personal team is created
                  automatically.
                </p>
              </div>
              ...
              <div className="group border-l border-[#2A2722] pl-8 py-2 hover:border-[#B09E80] transition-colors">
                <h3 className="font-display text-2xl font-medium text-[#F0E6D2]">
                  2. Create an agent &amp; get an API key
                </h3>
                <p className="mt-3 text-[#D4C5B0] opacity-70 leading-relaxed font-sans">
                  Go to{" "}
                  <strong className="text-[#F0E6D2]">
                    Team Settings → Add Agent
                  </strong>
                  . Give it a name, then copy the{" "}
                  <code className="text-[#B09E80] bg-[#B09E80]/10 px-1.5 py-0.5 rounded font-mono">
                    pb_...
                  </code>{" "}
                  API key. It&apos;s shown once — save it securely.
                </p>
              </div>
              ...
              <div className="group border-l border-[#2A2722] pl-8 py-2 hover:border-[#B09E80] transition-colors">
                ...
                <p className="mt-3 mb-5 text-[#D4C5B0] opacity-70 leading-relaxed font-sans">
                  Add this to your agent&apos;s MCP server configuration (works
                  with Warp, Cursor, Claude Code, or any MCP client):
                </p>
                <Code copyable>{MCP_CONFIG}</Code>
                <p className="mt-4 text-[#D4C5B0] opacity-50 text-sm font-sans italic">
                  Replace{" "}
                  <code className="text-[#B09E80]">pb_YOUR_API_KEY</code> with
                  your actual key.
                </p>
              </div>
              ...
              <div className="group border-l border-[#2A2722] pl-8 py-2 hover:border-[#B09E80] transition-colors">
                ...
                <p className="mt-3 text-[#D4C5B0] opacity-70 leading-relaxed font-sans">
                  ...
                  at the start of every session to orient itself.
                </p>
              </div>
            </div>
          </section>
          ...
          <section id="cli" className="mb-24">
            ...
            <p className="mt-4 text-lg text-[#D4C5B0] opacity-70 leading-relaxed font-sans max-w-2xl">
              ...
              CLI lets you manage tasks, knowledge, and agents from the terminal
              — and run the collaborator loop that dispatches work to AI agents.
            </p>
            ...
            <div className="mt-12 space-y-12">
              ...
              <div className="group border-l border-[#2A2722] pl-8 py-2 hover:border-[#B09E80] transition-colors">
                ...
                <p className="mt-3 mb-5 text-[#D4C5B0] opacity-70 leading-relaxed font-sans">
                  Authenticate with Google or GitHub (opens a browser), or paste
                  an API key directly.
                </p>
              ...
              <div className="group border-l border-[#2A2722] pl-8 py-2 hover:border-[#B09E80] transition-colors">
                ...
                <div className="space-y-3">
                  <Code copyable>
                    pb tasks list --project &lt;project-id&gt;
                  </Code>
              ...
              <div className="group border-l border-[#2A2722] pl-8 py-2 hover:border-[#B09E80] transition-colors">
                ...
                <p className="mt-3 mb-5 text-[#D4C5B0] opacity-70 leading-relaxed font-sans">
                  Start the collaborator loop — the runner polls for queued
                  tasks and dispatches them to the chosen agent. GitHub mode
                  (Docker + PR workflow) is enabled automatically when the
                  project has GitHub configured.
                </p>
                <Code copyable>
                  pb run --project &lt;project-id&gt; --agent &lt;name&gt;
                </Code>
                <p className="mt-4 text-[#D4C5B0] opacity-50 text-sm font-sans italic">
                  Omit <code className="text-[#B09E80]">--agent</code> to pick
              ...
              <div id="demo" className="scroll-mt-24">
                <h3 className="font-display text-2xl font-medium text-[#F0E6D2] mb-4">
                  Demo: autonomous agent walkthrough
                </h3>
                <p className="mb-6 text-[#D4C5B0] opacity-70 leading-relaxed font-sans">
                  End-to-end example: create an agent, wire up a project,
                  assign a workflow stage, create a task, and watch{" "}
                  <code className="text-[#B09E80] bg-[#B09E80]/10 px-1.5 py-0.5 rounded font-mono">
                    pb supervise
                  </code>{" "}
                  dispatch it — the agent implements and opens a pull request
                  without any human intervention.
                </p>
                <div className="overflow-hidden rounded-2xl border border-[#2A2722] bg-[#0A0A0B] shadow-2xl">
                  <video
                ...
                <div className="mt-10 space-y-8">
                  {[
                    ...
                    <div key={step} className="flex gap-6">
                      ...
                      <div className="pt-1">
                        <h3 className="font-display text-xl font-medium text-[#F0E6D2]">
                          {title}
                        </h3>
                        <p className="mt-2 leading-relaxed text-[#D4C5B0] opacity-70 font-sans">
                          {body}
                        </p>
                      </div>
                    </div>
              ...
              <div className="rounded-2xl border border-[#2A2722] bg-[#141311] p-8 shadow-xl">
                <h3 className="font-display text-2xl font-medium text-[#F0E6D2] mb-6">
                  All commands
                </h3>
                <div className="space-y-3">
                  {[
                    ...
                    <div
                      key={cmd as string}
                      className="flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-6 rounded-xl border border-[#2A2722] bg-[#0A0A0B] px-5 py-3"
                    >
                      <code className="font-mono text-sm text-[#B09E80] shrink-0">
                        {cmd}
                      </code>
                      <span className="text-[#D4C5B0] opacity-60 text-sm font-sans">
                        {desc}
                      </span>
                    </div>
                  ))}
          ...
          <section id="claude-code-skill" className="mb-24">
            ...
            <p className="mt-4 text-lg text-[#D4C5B0] opacity-70 leading-relaxed font-sans max-w-2xl">
              The official Claude Code skill gives Claude deep, built-in
              knowledge of Project Brain workflows, tool APIs, and
              knowledge-capture patterns — without pasting a system prompt.
            </p>
            ...
            <div className="mt-12 space-y-12">
              ...
              <div className="rounded-2xl border border-[#2A2722] bg-[#141311] p-8 shadow-xl">
                ...
                <p className="text-[#D4C5B0] opacity-70 mb-6 font-sans">
                  Once installed, Claude Code automatically loads the skill when
                  you mention Project Brain or any of its tools.
                </p>
                <ul className="space-y-4">
                  {[
                    {
                      label: "Session start workflow",
                      desc: 'reminds Claude to call context(action="session") before acting',
                    },
                    {
                      label: "Proactive knowledge logging",
                      desc: "guidance on when and how to create facts, decisions, and skills",
                    },
                    {
                      label: "Complete tool API reference",
                      desc: "all six MCP tools with every action and parameter",
                    },
                    {
                      label: "Knowledge quality patterns",
                      desc: "examples of good vs. bad entries and anti-patterns",
                    },
                    {
                      label: "Agent handoff workflow",
                      desc: "how to leave clean state when handing off to another agent",
                    },
                  ].map((item) => (
                    <li key={item.label} className="flex gap-4 items-start">
                      <span className="text-[#B09E80] mt-1 shrink-0">→</span>
                      <span className="text-[#D4C5B0] leading-relaxed font-sans">
                        <strong className="text-[#F0E6D2] font-medium">
                          {item.label}
                        </strong>{" "}
                        — {item.desc}
                      </span>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {[
                    ...
                    <div
                      key={phrase}
                      className="rounded-xl border border-[#2A2722] bg-[#0A0A0B] px-5 py-3 font-mono text-sm text-[#D4C5B0] opacity-60"
                    >
                      &ldquo;{phrase}&rdquo;
                    </div>
                  ))}
          ...
          <section id="system-prompt" className="mb-24">
            ...
            <p className="mt-4 text-lg text-[#D4C5B0] opacity-70 leading-relaxed font-sans max-w-2xl">
              Copy this into your agent&apos;s system prompt or rules file. It
              gives the agent full context on how to use ProjectBrain.
            </p>
            <div className="mt-12">
              <Code copyable wrap>
                {SYSTEM_PROMPT}
          ...
          <section id="workflow" className="mb-24">
            ...
            <p className="mt-4 text-lg text-[#D4C5B0] opacity-70 leading-relaxed font-sans max-w-2xl">
              The recommended workflow for agents using ProjectBrain. Follow
              these steps every session.
            </p>

            <div className="mt-12 space-y-8">
              <div className="rounded-2xl border border-[#2A2722] bg-[#141311] p-10 shadow-xl">
                <h3 className="mb-8 font-mono text-sm font-bold uppercase tracking-widest text-[#B09E80] opacity-60">
                  The Session Loop
                </h3>
                <div className="space-y-10">
                  {[
                    {
                      step: "1",
                      code: 'context(action="session", project_id)',
                      desc: "orient yourself: tasks, decisions, facts, skills, and team.",
                    },
                    {
                      step: "2",
                      code: 'tasks(action="list", status="todo")',
                      desc: "pick work from the backlog.",
                    },
                    {
                      step: "3",
                      code: 'tasks(action="update", status="in_progress")',
                      desc: "claim the task and notify the team.",
                    },
                    {
                      step: "4",
                      code: 'knowledge(entity="...", action="create")',
                      desc: "record tradeoffs, facts, and skills as you discover them.",
                    },
                    {
                      step: "5",
                      code: 'tasks(action="update", status="done")',
                      desc: "verify changes and ship the work.",
                    },
                  ].map((item) => (
                    <div key={item.step} className="flex gap-6">
                      ...
                      <div>
                        <code className="block mb-2 font-mono text-base text-[#F0E6D2]">
                          {item.code}
                        </code>
                        <p className="text-[#D4C5B0] opacity-70 font-sans">
                          {item.desc}
                        </p>
                      </div>
                    </div>
              ...
              <div className="grid gap-8 lg:grid-cols-2">
                <div className="rounded-2xl border border-[#2A2722] bg-[#141311] p-8 shadow-xl">
                  <h3 className="mb-6 font-mono text-sm font-bold uppercase tracking-widest text-[#B09E80] opacity-60">
                    Core Rules
                  </h3>
                  <ul className="space-y-4 text-[#D4C5B0] opacity-70 font-sans">
                    <li className="flex gap-3">
                      <span>•</span>{" "}
                      <span>
                        Always start with a full session context load.
                      </span>
                    </li>
                    <li className="flex gap-3">
                      <span>•</span>{" "}
                      <span>
                        Check existing skills before starting unfamiliar work.
                      </span>
                    </li>
                    <li className="flex gap-3">
                      <span>•</span>{" "}
                      <span>
                        Record decisions as tradeoffs (include rationale).
                      </span>
                    </li>
                    <li className="flex gap-3">
                      <span>•</span>{" "}
                      <span>Update task status proactively as you work.</span>
                    </li>
                    <li className="flex gap-3">
                      <span>•</span>{" "}
                      <span>
                        Read task history if you are picking up in-flight work.
                      </span>
                    </li>
                  </ul>
                </div>
                <div className="rounded-2xl border border-[#2A2722] bg-[#141311] p-8 shadow-xl">
                  ...
                  <dl className="space-y-5">
                    <div>
                      <dt className="text-[#F0E6D2] font-display font-medium mb-1">
                        Facts
                      </dt>
                      <dd className="text-sm text-[#D4C5B0] opacity-60 font-sans">
                        Durable rules, conventions, and project constraints.
                      </dd>
                    </div>
                    <div>
                      <dt className="text-[#F0E6D2] font-display font-medium mb-1">
                      ...
                      <dd className="text-sm text-[#D4C5B0] opacity-60 font-sans">
                        Specific architectural or design choices with reasoning.
                      </dd>
                    </div>
                    <div>
                      <dt className="text-[#F0E6D2] font-display font-medium mb-1">
                      ...
                      <dd className="text-sm text-[#D4C5B0] opacity-60 font-sans">
                        Reusable procedures and multi-step workflows for future
                        sessions.
                      </dd>
                    </div>
                  </dl>
          ...
          <section id="gherkin-style" className="mb-24">
            ...
            <p className="mt-4 text-lg text-[#D4C5B0] opacity-70 leading-relaxed font-sans max-w-2xl">
              ProjectBrain uses the Given/When/Then Gherkin structure for all
              agent prompts to maximize determinism and reliability.
            </p>
            ...
            <div className="mt-12 grid gap-8 md:grid-cols-3">
              {[
                {
                  title: "Given",
                  desc: "Establishes the persona, state, and available tools before action.",
                },
                {
                  title: "When",
                  desc: "Defines the exact trigger or condition that causes an action.",
                },
                {
                  title: "Then",
                  desc: "Specifies the precise outcome, format, and behavior expected.",
                },
              ].map((item) => (
                <div
                  key={item.title}
                  className="rounded-2xl border border-[#2A2722] bg-[#141311] p-8"
                >
                  <h3 className="font-mono text-xl font-bold text-[#B09E80] mb-3">
                    {item.title}
                  </h3>
                  <p className="text-sm text-[#D4C5B0] opacity-60 font-sans leading-relaxed">
                    {item.desc}
                  </p>
                </div>
              ))}
            ...
            <div className="mt-12 rounded-2xl border border-[#B09E80]/20 bg-[#B09E80]/5 p-8 font-sans">
              <h3 className="font-display text-2xl text-[#F0E6D2] mb-4">
                Best Practices
              </h3>
              <ul className="space-y-4 text-[#D4C5B0] opacity-80">
                <li className="flex gap-4">
                  ...
                  <span>
                    ...
                    — Tell the agent what to do instead of what to avoid.
                  </span>
                </li>
                <li className="flex gap-4">
                  ...
                  <span>
                    ...
                    — Use explicit scenarios to handle happy paths vs error
                    branches.
                  </span>
                </li>
                <li className="flex gap-4">
                  ...
                  <span>
                    ...
                    — Be highly specific about the structure and tone of the
                    response.
                  </span>
                </li>
              </ul>
...
Then I will:
  ...
  5. Post a structured review summary: Approved / Changes Requested / Blocked`}</pre>
            ...
            <div className="mt-12 space-y-12">
              {TOOL_GROUPS.map(({ group, tools }) => (
                <div key={group} className="relative">
                  <h3 className="mb-6 text-xs font-mono font-bold uppercase tracking-[0.2em] text-[#B09E80] opacity-50">
                    {group}
                  </h3>
                  <div className="space-y-4">
                    {tools.map(([tool, desc]) => {
                      ...
                      return (
                        <div
                          key={tool}
                          className="rounded-2xl border border-[#2A2722] bg-[#141311] shadow-lg transition-all hover:border-[#B09E80]/20"
                        >
                          <div className="p-6">
                            <div className="mb-4 overflow-x-auto rounded-xl border border-[#2A2722] bg-[#0A0A0B] p-4">
                              <code className="font-mono text-sm leading-relaxed text-[#B09E80] whitespace-pre-wrap break-words">
                                {toolLabel}
                              </code>
                            </div>
                            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                              <span className="text-[#D4C5B0] opacity-70 font-sans">
                                {desc}
                              </span>
                              {hasParams && (
                                <button
                          ...
                          {hasParams && isExpanded && (
                            <div className="border-t border-[#2A2722] bg-[#0A0A0B]/50 p-6 rounded-b-2xl">
                              <p className="mb-4 text-xs font-mono font-bold uppercase tracking-widest text-[#B09E80] opacity-40">
                                Parameters
                              </p>
                              <div className="grid gap-4">
                                {params.map((param) => (
                                  <div
                                    key={param.name}
                                    className="flex flex-col sm:flex-row sm:gap-6 border-b border-[#2A2722]/50 pb-4 last:border-0 last:pb-0"
                                  >
          ...
          <section id="curation" className="mb-24">
            ...
            <p className="mt-4 text-lg text-[#D4C5B0] opacity-70 leading-relaxed font-sans max-w-2xl">
              An automated curator periodically reviews your knowledge base for
              duplicates and quality issues, surfacing suggestions in the UI.
            </p>
                  ...
                  {[
                    {
                      ...
                      desc: "Combines semantically equivalent entries into one canonical record.",
                    },
                    {
                      label: "ARCHIVE",
                      color: "bg-orange-500/20 text-orange-300",
                      desc: "Removes stale or irrelevant knowledge that is no longer useful.",
                    },
                    {
                      label: "REFRESH",
                      color: "bg-teal-500/20 text-teal-300",
                      desc: "Confirms an entry is still current to prevent staleness warnings.",
                    },
                    {
                      label: "FLAG",
                      color: "bg-yellow-500/20 text-yellow-300",
                      desc: "Identifies quality issues like vague titles or missing rationale.",
                    },
                    {
                      label: "SUPERSEDE",
                      color: "bg-purple-500/20 text-purple-300",
                      desc: "Links an old entry to a newer, more relevant replacement.",
                    },
                  ].map((type) => (
                    <div key={type.label} className="flex flex-col gap-3">
                      <span
                        className={`w-fit rounded px-2 py-0.5 text-xs font-bold font-mono tracking-widest ${type.color}`}
                      >
                        {type.label}
                      </span>
                      <p className="text-sm text-[#D4C5B0] opacity-60 font-sans leading-relaxed">
